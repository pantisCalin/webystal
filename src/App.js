...
import { atoms } from "./components/AllAtoms";
...
function App() {
  ...
  // List of Constants
  const [angles, setAngles] = useState([29, 104]);
  ...
  const [dataToPlotX, setDataToPlotX] = useState([
    0, 0, 0, 2.5, 2.5, 0, 2.5, 2.5, 1.25,
  ]);
  const [dataToPlotY, setDataToPlotY] = useState([
    0, 0, 2.5, 2.5, 0, 2.5, 2.5, 0, 1.25,
  ]);
  const [dataToPlotZ, setDataToPlotZ] = useState([
    0, 2.5, 2.5, 0, 2.5, 0, 2.5, 0, 1.25,
  ]);
  const [colors, setColors] = useState(allColors);
  const [speciesAtoms, setSpeciesAtoms] = useState([1, 1, 1, 1, 1, 1, 1, 1, 6]);
  const [species, setSpecies] = useState([1, 6]);
  const [selectedPoint, setSelectedPoint] = useState(-1);
  const [removedData, setRemovedData] = useState([]);
  const [typeOfDataRotation, setTypeOfDataRotation] = useState("none");
  const [angleRotatedData, setAngleRotatedData] = useState(1);

  const onChangeColor = (index, value) => {
    setColors([...colors.slice(0, index), value, ...colors.slice(index + 1)]);
    ...
    for (let fil = 0; fil < length; fil++) {
      ...
      reader.onload = (e) => {
        ...
        primVec.forEach((element) => {
          let row = element
            .split(" ")
            .filter((fileElem) => fileElem.length > 0);
          primitiveVectors = [
            ...primitiveVectors,
    ...
    for (let i = 0; i < arrayX.length; i++) {
      ...
      if (state.repetitionX[atomIndex] !== 0) {
        for (let j = 0; j < state.repetitionX[atomIndex]; j++) {
          xForPush = [
            ...xForPush,
            arrayX[i] / 1 + state.constantX[atomIndex][0] * j,
          ];
          yForPush = [
            ...yForPush,
            arrayY[i] / 1 + state.constantX[atomIndex][1] * j,
          ];
          zForPush = [
            ...zForPush,
            arrayZ[i] / 1 + state.constantX[atomIndex][2] * j,
          ];
    ...
    for (let i = 0; i < arrayY.length; i++) {
      ...
      if (state.repetitionY[atomIndex] !== 0) {
        for (let j = 0; j < state.repetitionY[atomIndex]; j++) {
          xForPush = [
            ...xForPush,
            arrayX[i] / 1 + state.constantY[atomIndex][0] * j,
          ];
          yForPush = [
            ...yForPush,
            arrayY[i] / 1 + state.constantY[atomIndex][1] * j,
          ];
          zForPush = [
            ...zForPush,
            arrayZ[i] / 1 + state.constantY[atomIndex][2] * j,
          ];
          specForPush = [...specForPush, speciesMulti[i]];
        }
      }
    ...
    for (let i = 0; i < arrayZ.length; i++) {
      ...
      if (state.repetitionZ[atomIndex] !== 0) {
        for (let j = 0; j < state.repetitionZ[atomIndex]; j++) {
          xForPush = [
            ...xForPush,
            arrayX[i] / 1 + state.constantZ[atomIndex][0] * j,
          ];
          yForPush = [
            ...yForPush,
            arrayY[i] / 1 + state.constantZ[atomIndex][1] * j,
          ];
          zForPush = [
            ...zForPush,
            arrayZ[i] / 1 + state.constantZ[atomIndex][2] * j,
          ];
          specForPush = [...specForPush, speciesMulti[i]];
        }
      }
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < dataMultiX.length; i++) {
      ...
      datXHere = [
        ...datXHere,
        dataMultiX[i] / 1 + state.translateX[trIndex] / 1,
      ];
      datYHere = [
        ...datYHere,
        dataMultiY[i] / 1 + state.translateY[trIndex] / 1,
      ];
      datZHere = [
        ...datZHere,
        dataMultiZ[i] / 1 + state.translateZ[trIndex] / 1,
      ];
    }
    ...
    switch (typeOfDataRotation) {
      case "x":
        let xMatrixRot = [
          ...
          [
            0,
            Math.cos((angleRotatedData * Math.PI) / 180),
            -Math.sin((angleRotatedData * Math.PI) / 180),
          ],
          [
            0,
            Math.sin((angleRotatedData * Math.PI) / 180),
  ...
  const onAddData = (e) => {
    ...
    if (removedData.includes(index)) {
      ...
      setRemovedData([
        ...removedData.slice(0, positionRemoved),
        ...removedData.slice(positionRemoved + 1),
      ]);
    }
  };
  const onChangeDataRotationType = (e) => {
    setTypeOfDataRotation(e.target.value);
    setAngleRotatedData(1);
  };
    ...
    <div className="App">
      ...
      <div className="underNav">
        ...
        <div className="ControlPannel">
          <UploadFiles onUploadedFile={(e) => onUploadedFile(e)} />
          <Rotations
            stretches={stretches}
          ...
          <div className="oneSpec">
            <div className="specName"> Pannel</div>
            <div className="DFR">
              <button
