...
const DataPoints = ({
  ...
  for (let i = 0; i < dataX.length; i++) {
    rowsForTable = [
      ...rowsForTable,
      [[allMultiSpec[i], dataX[i], dataY[i], dataZ[i]], i],
    ];
  }
  ...
  const downloadSpecies = (e) => {
    let txt =
      "# Thank you for using webystal. Your data is ready to be inserted into the fdf file\n\n";
    txt =
      txt +
      `NumberOfSpecies ${species.length}\nNumberOfAtoms ${
        finalRows.length - removedData.length
    ...
    for (let i = 0; i < lattice.length; i++) {
      let xThisRow =
        lattice[i][0] * multipliers[i] >= 0
          ? (lattice[i][0] * multipliers[i]).toFixed(9)
          : (lattice[i][0] * multipliers[i]).toFixed(8);
      let yThisRow =
        lattice[i][1] * multipliers[i] >= 0
          ? (lattice[i][1] * multipliers[i]).toFixed(9)
          : (lattice[i][1] * multipliers[i]).toFixed(8);
      let zThisRow =
        lattice[i][2] * multipliers[i] >= 0
          ? (lattice[i][2] * multipliers[i]).toFixed(9)
          : (lattice[i][2] * multipliers[i]).toFixed(8);
    ...
    for (let i = 0; i < finalRows.length; i++) {
      ...
      if (removedData.includes(finalRows[i][1])) {
        ...
        let dataXHere =
          finalRows[i][0][1] >= 0
            ? finalRows[i][0][1].toFixed(9)
            : finalRows[i][0][1].toFixed(8);
        let dataYHere =
          finalRows[i][0][2] >= 0
            ? finalRows[i][0][2].toFixed(9)
            : finalRows[i][0][2].toFixed(8);
        let dataZHere =
          finalRows[i][0][3] >= 0
            ? finalRows[i][0][3].toFixed(9)
            : finalRows[i][0][3].toFixed(8);
  ...
  const downloadXSF = (e) => {
    let txt =
      " # crystal structure from Webystal\n CRYSTAL\n # Cell vectors in Angstroem:\n PRIMVEC\n";
    let lattice = [state.constantX[0], state.constantY[0], state.constantZ[0]];
    let multipliers = [
      state.repetitionX[0],
      state.repetitionY[0],
    ...
    for (let i = 0; i < finalRows.length; i++) {
      if (removedData.includes(finalRows[i][1])) {
        ...
        let row = `  ${finalRows[i][0][0]}        ${finalRows[i][0][1].toFixed(
          8
        )}        ${finalRows[i][0][2].toFixed(8)}        ${finalRows[
          i
        ][0][3].toFixed(8)}\n`;
        txt = txt + row;
      }
  ...
  return (
    <div className="oneSpec">
      <div className="specName">
        <div className="tableTitle">Data Table</div>
        <div>
            ...
            <tr key={indElem}>
              {element[0].map((val, indVal) => (
                <td
                  key={indVal}
                  className={
                    selectedPoint === element[1]
                      ? "focusedData"
                      : removedData.includes(element[1])
                      ? "redSelected"
                      : ""
                  }
                  onClick={() => dataClicked(element[1])}
                >
      ...
      <button className="download" onClick={downloadXSF}>
        Download XSF
      </button>
      <button className="download" onClick={downloadSpecies}>
        Download Species
      </button>
    </div>
  );
